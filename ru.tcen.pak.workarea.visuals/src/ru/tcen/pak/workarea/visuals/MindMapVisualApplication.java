...
//		MindMapNodeVisual node2 = new MindMapNodeVisual(new Rectangle(0, 0, 100, 100), 6, false, false);
...
//		conn.setEndAnchor(new DynamicAnchor(node2, new ChopBoxStrategy()));
		...
		root.getChildren().addAll(conn, node);

		System.out.println(root.getChildren().get(1));
		System.out.println(root.getChildren().get(1).getBoundsInParent());
		System.out.println(root.getChildren().get(1).getBoundsInLocal());
